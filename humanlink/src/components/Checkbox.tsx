...
interface CheckboxProps extends React.InputHTMLAttributes<HTMLInputElement> {
    label?: string;
    sublabel?: string; // Helpful for things like "Rest Day"
}
...
export default function Checkbox({ label, sublabel, checked, className = "", ...props }: CheckboxProps) {
    return (
        <label className={`flex flex-col items-center justify-center gap-1.5 cursor-pointer group ${className}`}>
            {/* The Top Label (Matching your Pro SaaS FormLabel style) */}
            {label && (
            ...
            <div className="relative flex items-center justify-center">
                ...
                {/* The Custom Checkmark (Visible only when checked) */}
                <Check 
                    size={14} 
                    strokeWidth={4} 
            ...
            {/* Optional Sublabel (Small italic/muted text if needed) */}
            {sublabel && (
